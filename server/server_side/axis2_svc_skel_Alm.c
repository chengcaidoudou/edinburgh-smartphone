				...
				axis2_msg_ctx_t *msg_ctx)
	{
          ...
          */
          ...
          if (op_name)
          {
                ...
                if ( axutil_strcmp(op_name, "optimizePortfolioSSD") == 0 )
                {

                    
                    input_val1 = adb_optimizePortfolioSSD_create( env);
                        if( AXIS2_FAILURE == adb_optimizePortfolioSSD_deserialize(input_val1, env, &content_node, NULL, AXIS2_FALSE))
                        {
                            adb_optimizePortfolioSSD_free(input_val1, env);
                      
                            AXIS2_ERROR_SET(env->error, AXIS2_ERROR_DATA_ELEMENT_IS_NULL, AXIS2_FAILURE);
                            AXIS2_LOG_ERROR( env->log, AXIS2_LOG_SI, "NULL returnted from the adb_optimizePortfolioSSD_deserialize: "
                                        "This should be due to an invalid XML");
                            return NULL;      
                        }
                        
                        ret_val1 =  axis2_skel_Alm_optimizePortfolioSSD(env, msg_ctx,input_val1);
                  ...
                  const axutil_env_t *env, axiom_node_t *node)
	{
        ...
        error_code = axutil_error_codes_t(env->error->error_number);
